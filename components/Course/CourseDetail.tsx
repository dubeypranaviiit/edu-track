...
// import { CourseType } from "@/types/TypeCourse"; // Make sure this matches updated model
        ...
        <div className="container mx-auto px-4 py-20">
          ...
          <div className="flex items-center space-x-4 mb-6">
            {[...Array(5)].map((_, i) => (
              <FaStar
                key={i}
                // className={`w-5 h-5 ${i < Math.round(averageRating) ? "text-yellow-400" : "text-gray-400"}`}
              />
            ))}
                  ...
                  <div className="mt-2 ml-4 p-4 bg-gray-700 rounded-lg space-y-2">
                    {chapter.subtopics.map((subtopic, i) => (
                      <div key={subtopic._id || i}>
                        <div className="text-lg font-medium mb-1">{subtopic.title}</div>
                        {subtopic.items.map((item, j) => (
                          <div key={item._id || j} className="flex items-center space-x-2 pl-4 py-1">
                            <FaVideo className="text-blue-400" />
